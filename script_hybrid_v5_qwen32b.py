...
MODEL_EXTRACT = "qwen2.5:32b"
MODEL_REASON = "qwen2.5:32b"
INPUT_FILE = "LLM_input_timeline_annotated_anonymized.csv"
OUTPUT_FILE = "output_hybrid_soft_rule_v5_32b.csv"
ERROR_LOG = "error_log_hybrid_soft_rule_v5_32b.txt"
MAX_RETRIES = 3
TIMEOUT = 600
...
# =========================
# Step 3: Soft Rule Layer (v4)
#
# Design principles:
#   - Rules NEVER override LLM's is_spc judgment
#   - Rules only adjust Evidence_level (downgrade by 1 step) as advisory signals
#   - Exception: single tumor → hard override (structurally 100% certain)
#   - The goal is to flag uncertainty, not to force decisions
# =========================
...
def downgrade_evidence(level: str) -> str:
    """Downgrade evidence level by one step: A→B, B→C, C stays C."""
    if level == "A":
        return "B"
    elif level == "B":
        return "C"
...
def apply_soft_rule(tumors: list, llm_result: Dict[str, Any]) -> Tuple[Dict[str, Any], list]:
    """
    Soft rule layer (v4).

    Key difference from hard rule:
    - Rules NEVER override is_spc (except single tumor which is 100% certain)
    - Rules only downgrade Evidence_level as an advisory signal
    - LLM's clinical judgment is always preserved
    """
    result = dict(llm_result)
    rule_flags = []

    # ---- Rule 1: Single tumor → hard override (structurally certain) ----
    if len(tumors) < 2:
        result["is_spc"] = False
        result["SPC_type"] = 0
        result["reason"] += " [SOFT-RULE: only one tumor event — hard override]"
        rule_flags.append("single_tumor_hard")
    ...
    # ---- Rule 2: All same site + same histology → downgrade evidence ----
    if len(unique_sites) == 1 and len(unique_histologies) <= 1:
        old_ev = result.get("Evidence_level", "C") or "C"
        result["Evidence_level"] = downgrade_evidence(old_ev)
        result["reason"] += f" [SOFT-RULE: same site and histology — evidence downgraded {old_ev}→{result['Evidence_level']}]"
        rule_flags.append("same_primary_downgrade")
        # is_spc is NOT changed — LLM judgment preserved
        return result, rule_flags

    # ---- Rule 3: All same organ + same histology → downgrade evidence ----
    if len(unique_organs) == 1 and len(unique_histologies) <= 1:
        organ_name = list(unique_organs)[0]
        old_ev = result.get("Evidence_level", "C") or "C"
        result["Evidence_level"] = downgrade_evidence(old_ev)
        result["reason"] += f" [SOFT-RULE: same organ ({organ_name}) and histology — evidence downgraded {old_ev}→{result['Evidence_level']}]"
        rule_flags.append("same_organ_downgrade")
        # is_spc is NOT changed — LLM judgment preserved
        return result, rule_flags

    # ---- Rule 4: Multi-organ + multi-histology → advisory (upgrade evidence) ----
    if len(unique_organs) > 1 and len(unique_histologies) > 1:
        old_ev = result.get("Evidence_level", "C") or "C"
        # Only upgrade if LLM already said SPC — reinforce confidence
        if result.get("is_spc") == True and old_ev == "C":
            result["Evidence_level"] = "B"
            result["reason"] += f" [SOFT-RULE: multi-organ multi-histology supports SPC — evidence upgraded {old_ev}→B]"
            rule_flags.append("multi_organ_upgrade")
        else:
            result["reason"] += " [SOFT-RULE: multi-organ multi-histology noted]"
            rule_flags.append("multi_organ_advisory")
        return result, rule_flags

    # ---- No structural rule matched → pass through unchanged ----
...
def main():
    ...
    for _, row in tqdm(df.iterrows(), total=len(df)):
        ...
        final, rule_status = apply_soft_rule(tumors, llm_result)
